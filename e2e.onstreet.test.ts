...
describe('On-Street Parking Flow', () => {
    beforeAll(async () => {
        await device.launchApp({ newInstance: true, permissions: { location: 'always' } });
        await loginWithOtp();
    });

    it('opens the on-street parking screen via deep link', async () => {
        await device.openURL({ url: 'nrjsoft://onstreet' });

        await waitFor(element(by.text('On-Street Parking')))
            .toBeVisible()
            .withTimeout(8000);
    });
});
